"""Subject detail content assembly (Second Brain + Google Classroom).

Everything the Subject Details page needs comes from real Second Brain /
Classroom data — nothing here is mocked:

- ``course_documents`` — KB documents linked to the course's ``course:*`` tag
  (reuses the derivation logic from the courses router).
- ``build_topic_tree`` — nested topics/subtopics built from the documents'
  stored outlines (heading structure), with the documents that actually
  contain each heading attached to that topic.
- ``course_concepts`` — concepts mentioned (MENTIONS edges) by the subject's
  documents, ranked by mention frequency, with their source documents.
- ``course_graph`` — subject-scoped knowledge graph (only nodes/edges touching
  the subject's documents), reusing ``build_graph``.
- ``course_gaps`` — gap analysis scoped to the subject: topic coverage from
  the topic tree, concept-level gaps (memory/errors/misses) restricted to the
  subject's concepts, and Classroom assignment signals.
- ``course_content`` — the aggregate payload consumed by
  ``GET /api/courses/{id}/content``.

Every function is user-scoped via ``user_id``.
"""
...
def course_documents(
    ...
    *,
    ...
    """Return the Second Brain documents linked to this course.
    ...
    folder_ids = folder_data[1]
    doc_ids: set[int] = set(folder_ids)
...
def course_document_count(db: Session, user_id: int, course: Course) -> int:
    ...
    Mirrors ``course_documents`` resolution exactly — folder path, ``kb_tag_id``
    ...
    _roots, folder_ids = _course_folder_data(db, user_id, course.title)
    doc_ids: set[int] = set(folder_ids)
    ...
    if course.kb_tag_id:
        ...
        if matching_tags:
            doc_ids.update(
                r[0]
                for r in db.query(KbDocumentTag.document_id)
                .filter(KbDocumentTag.tag_id.in_([t[0] for t in matching_tags]))
                .distinct()
                .all()
            )
    return len(doc_ids)
...
def _foldered_outline(doc: dict[str, Any], folder_root: str) -> list[dict[str, Any]]:
    ...
    if not segments:
        return outline
    depth = len(segments)
    # Synthetic folder entries (``char_start`` is meaningless here — only used
    # for document outline display, never for these derived topics). ``origin``
    # marks them as vault-folder topics so the tree builder (and UI) can show
    # a folder path breadcrumb. A root heading that happens to share a folder's
    # name merges with it in the tree builder (dedupe by normalized name) —
    # intentional, both mean the same.
    prepend = [
    ...
    *,
    ...
    def _node(
        ...
        node = {
            ...
            "name": name,
            "level": level,
            # folder → this topic is a vault folder under the subject
            # (breadcrumb origin); heading → built from document headings.
            "origin": origin,
            "documents": [],
            "children": [],
        }
    ...
    for doc in documents:
        outline = _foldered_outline(doc, folder_root) if folder_root else (doc.get("outline") or [])
        # Stack of (container, level) pairs; starts at the root level 0.
        ...
        for item in outline:
            ...
            if not name or not _normalize_heading(name):
                continue
            # Pop until we're back at the parent of this level.
            while stack and stack[-1][1] >= level:
                stack.pop()
            container = stack[-1][0] if stack else roots

            # Merge with an existing sibling of the same normalized name.
            ...
            if node is None:
                ...
                # The same topic came from a vault folder AND a heading — the
                # folder reading wins: it IS a folder in the vault, so the UI
                # shows it with the folder breadcrumb.
                node["origin"] = "folder"

            node["documents"].append(doc)
            stack.append((node["children"], level))

    # Deterministic ordering + document dedupe per topic.
...
def course_gaps(db: Session, user_id: int, course: Course) -> dict[str, Any]:
    """Gap analysis for the subject, from actual data.

    Legacy (kept for backward compatibility):
    - ``topics`` — topic-level coverage gaps (headings with few documents).
    - ``concepts`` — concept-level gaps scoped to the subject's concepts.
    - ``classroom`` — Classroom assignment signals for the course.
    - ``threshold`` / ``document_count``.

    New actionable engine (the redesigned Gap Analysis):
    - ``summary`` — narrative + priority list (what to do next).
    - ``strengths`` — concepts with real evidence of mastery.
    - ``gaps`` — ranked missing/weak concepts with why, prerequisites,
      learn/practice steps, next, and linked Second Brain resources.
    - ``path`` — prerequisite-ordered learning phases.
    - ``next`` — the single highest-value next concept.
    - ``domain`` — the matched curated domain (if any).
    - ``coverage`` — secondary percentage (never the main output).
    """
    # Folder-derived subjects include their vault folders as topics (mirroring
    # ``course_content`` / ``build_topic_tree``) so those topics also surface in
    # the topic-coverage list AND the actionable engine's target set. A
    # tag-derived (or manual) subject whose title matches a vault folder gets
    # the same treatment: the folder's nested subfolders are knowledge targets.
    ...
    # Concept gaps scoped to the subject's mentioned concepts.
    ...
    # Subject-scoped only: never fall back to scoring the user's ENTIRE concept
    # catalog. Passing ``None`` would score every vault concept (an N+1 query
    # per concept) — the endpoint timed out on subjects with no documents, and
    # whole-vault gaps are not this subject's gaps. An empty set means the
    # subject mentions no concepts → no concept gaps.
    concepts = concept_gaps(
        db, user_id, limit=MAX_CONCEPT_GAPS, concept_ids=concept_ids
    )

    # New actionable engine — same subject, same data, richer output.
    # (legacy ``concept_gaps`` entries carry ``concept`` as the name field)
...
def course_content(db: Session, user_id: int, course: Course) -> dict[str, Any]:
    ...
    # A document is "organized" when it lands in at least one topic — it has
    # an outline (heading topics) or, for folder subjects, sits under a
    # nested folder (folder topics). Everything else is flat/unorganized.
    def _is_organized(doc: dict[str, Any]) -> bool:
        if doc.get("outline"):
            return True
        return bool(folder_root and _folder_segments(doc.get("path_rel"), folder_root))
    ...
    return {
        ...
        "second_brain": {
            ...
            "domains": domains,
            "unorganized_documents": [d for d in documents if not _is_organized(d)],
