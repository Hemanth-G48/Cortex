...
export const KnowledgeGraph = () => {
  ...
  // Derived graph data, memoized so the renderer effect only re-runs on new data.
  ...
  useEffect(() => {
    if (!graphData || fgNodes.length === 0) return;
    const container = containerRef.current;
    if (!container) return;

    const graph = new Graph();
    const nodeCount = fgNodes.length;
    fgNodes.forEach((n, i) => {
  ...
  }, [graphData, fgNodes, filteredLinks, nodeReducer, edgeReducer, handleEnterNode, handleLeaveNode, handleNodeClick]);
    ...
    <div className="page-section">
      ...
      {loading ? (
        ...
        >
          ...
          {/* Zoom / fit controls (bottom-right, Obsidian-style) */}
          <div
            style={{
              position: 'absolute',
              right: '0.5rem',
